...
namespace Poya.BasePlatform.WebNetCore.Infrastructure.Extensions
{
    ...
    public static class IDbConnectionExtensions
    {
        ...
        private static List<DbTable> GetCurrentDatabaseAllTables(this IDbConnection dbConnection, DatabaseType dbType)
        {
            ...
            var sqlcon = (MySqlConnection)dbConnection;
            MySqlCommand cmd = new MySqlCommand(dbConnection.strGetAllTablesSql(dbType), sqlcon);
            ...
            try
            {
                ...
                while (reader.Read())
                {
                    result.Add(new DbTable()
                    {
                        TableComment = reader.GetString("TableComment"),
        ...
        {
            var result = new List<DbTableColumn>();
            if (dbConnection == null) throw new ArgumentNullException(nameof(dbConnection));
            if (dbConnection.State == ConnectionState.Closed) dbConnection.Open();

            var sqlcon = (MySqlConnection)dbConnection;
            MySqlCommand cmd = new MySqlCommand(dbConnection.strGetAllColumnsSql(dbType, tableName), sqlcon);
            MySqlDataReader reader = null;
            try
            {
                //打开连接
                ...
                while (reader.Read())
                {
                    result.Add(new DbTableColumn()
                    {
                        ColName = reader.GetString("ColName"),
                        IsIdentity = reader.GetBoolean("IsIdentity"),
                        IsPrimaryKey = reader.GetBoolean("IsPrimaryKey"),
                        ColumnType = reader.GetString("ColumnType"),
                        ColumnLength = reader.GetInt64("ColumnLength"),
